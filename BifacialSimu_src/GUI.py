...
try:
    ...
    # Import of showinfo to get the event response of tkinter objects (is not used in the program yet)
...
# import modules needed for calculation of soiling rate
...
SimulationDict = {
    ...
    'simulationMode': 1,
    # Decide wether you want to use a  weather file or try to download one for the coordinates
    'localFile': True,
    # weather file in TMY format
    'weatherFile': rootPath + '/WeatherData/Golden_USA/SRRLWeatherdata Nov_Dez_2.csv',
    ...
    # Only for hourly simulation, yy, mm, dd, hh
    'startHour': (2019, 11, 1, 0),
    ...
    # Solar backtracking is a tracking control program that aims to minimize PV panel-on-panel shading
    'backTracking': False,
    # simple electrical Simulation after PVSyst, use if rear module parameters are missing
    'ElectricalMode_simple': False,
    'limitAngle': 60,  # limit Angle for singleAxisTracking
    'hub_height': 1.3,  # Height of the rotation axis of the tracker [m]
    # azimuth of the PV surface [deg] 90°: East, 135° : South-East, 180°:South
    'azimuth': 180,
...
ModuleDict = {
    ...
    # Short-circuit current measured for front side illumination of the module at STC [A]
    'I_sc_f': 9.5,
    # Short-circuit current measured for rear side illumination of the module at STC [A]
    'I_sc_r': 6.56,
    # Open-circuit voltage measured for front side illumination of module at STC [V]
    'V_oc_f': 48,
    # Open-circuit voltage measured for rear side illumination of module at STC [V]
    'V_oc_r': 47.3,
    ...
    # Ambient Temperature for measuring the Temperature Coeffizient [°C]
    'T_amb': 20,
...
class Window(tk.Tk):
    def __init__(self):
        ...
        def github_infopage(option_1):
            ...
            if option_1 == "yes":
                webbrowser.open(
                    "https://github.com/cire-thk/BifacialSimu#readme", new=1)
...
# =============================================================================
        ...
        def button_MC():
            ...
            Info_label = Label(pop, text=text_MP, bg="white",
            ...
            more_info_button = Button(
                info_frame, text='More Information', command=lambda: github_infopage("yes"))
        ...
        def button_SP():
            ...
            # global pop_SP
            pop_SP = Toplevel()
            pop_SP.title("Simulation Parameter Info!")
            ...
            SP_Info = Label(pop_SP, text=text_SP, bg="white",
                            font=("Arial", 8), justify="left")
            SP_Info.pack(pady=5)
...
# =============================================================================
        ...
        namecontrol_frame.infoButton_MC = PhotoImage(
            file=rootPath+'\Lib\Button_Images\Button-Info-icon.png')
...
# =============================================================================
        # Main Control tab
        Info_MC = Button(namecontrol_frame, image=Info_image,
                         command=button_MC, borderwidth=0)
        Info_MC.grid(row=0, column=1)
        # Simulation Control tab
        Info_SC = Button(simulationMode_frame, image=Info_image,
                         command=button_SC, borderwidth=0)
        Info_SC.grid(row=0, column=1)
        # Module Parameter tab
        Info_MP = Button(ModuleParameter_frame, image=Info_image,
                         command=button_MP, borderwidth=0)
        Info_MP.grid(row=0, column=2)
        # Simulation Parameter tab
        Info_SP = Button(simulationParameter_frame,
                         image=Info_image, command=button_SP, borderwidth=0)
        ...
        namecontrol_label = ttk.Label(
            namecontrol_frame, text='Main Control', font=("Arial Bold", 15))
        simulationMode_label = ttk.Label(
            simulationMode_frame, text='Simulation Control', font=("Arial Bold", 15))
        simulationParameter_label = ttk.Label(
            simulationParameter_frame, text='Simulation Parameter', font=("Arial Bold", 15))
        ModuleParameter_Label = ttk.Label(
            ModuleParameter_frame, text='Module Parameter', font=("Arial Bold", 15))
        ...
        def StartSimulation():
            ...
            if (len(Entry_year_start.get()) == 0 or len(Entry_month_start.get()) == 0
                ...
                messagebox.showwarning(
                    "Simulation Control", "Please insert a Start and End Date \n in the format: [yyyy mm dd hh]!")
                exit
            if int(Entry_month_start.get()) <= 0 or int(Entry_month_start.get()) > 12:
                messagebox.showwarning(
                    "Simulation Control", "Please insert a Start Month between 1 and 12!")
                exit
            if int(Entry_day_start.get()) < 1 or int(Entry_day_start.get()) > 31:
                messagebox.showwarning(
                    "Simulation Control", "Please insert a Start Day between 1 and 31!")
                exit
            if int(Entry_hour_start.get()) < 0 or int(Entry_hour_start.get()) >= 24:
                messagebox.showwarning(
                    "Simulation Control", "Please insert a Start Hour between 0 and 23!")
                exit
            if int(Entry_month_end.get()) <= 0 or int(Entry_month_end.get()) > 12:
                messagebox.showwarning(
                    "Simulation Control", "Please insert a End Month between 1 and 12!")
                exit

            if int(Entry_day_end.get()) < 1 or int(Entry_day_end.get()) > 31:
                messagebox.showwarning(
                    "Simulation Control", "Please insert a End Day between 1 and 31!")
                exit

            if int(Entry_hour_end.get()) < 0 or int(Entry_hour_end.get()) >= 24:
                messagebox.showwarning(
                    "Simulation Control", "Please insert a End Hour between 0 and 23!")
                exit
            ...
            if (len(Entry_year_start.get()) != 0
                ...
                Startdate = datetime.datetime(int(Entry_year_start.get()), int(Entry_month_start.get(
                )), int(Entry_day_start.get()), int(Entry_hour_start.get()))  # defining as Date
                SimulationDict["startHour"] = (
                    Startdate.year, Startdate.month, Startdate.day, Startdate.hour)
                Enddate = datetime.datetime(int(Entry_year_end.get()), int(
                    Entry_month_end.get()), int(Entry_day_end.get()), int(Entry_hour_end.get()))
                SimulationDict["endHour"] = (
...
# =============================================================================
            ...
            if len(Entry_ClearanceHeight.get()) != 0:
                SimulationDict["clearance_height"] = float(
                    Entry_ClearanceHeight.get())
                # Calculate the hub height of the PV rows, measured at the bottom edge
                SimulationDict['hub_height'] = (SimulationDict['clearance_height'] + (
                    math.sin(SimulationDict['tilt'])*SimulationDict['moduley']/2))
            ...
            if len(Entry_frontReflect.get()) != 0:
                SimulationDict["frontReflect"] = float(
                    Entry_frontReflect.get())
            ...
            if len(Entry_HubHeight.get()) != 0:
                ...
                SimulationDict['clearance_height'] = (
                    SimulationDict['hub_height'] - (math.sin(SimulationDict['tilt'])*SimulationDict['moduley']/2))
        ...
        def makePlotAbsIrr(resultsPath):
            if SimulationDict["simulationMode"] == 1 or SimulationDict["simulationMode"] == 2:
               ...
               # print (timestamp_start)
                # Counting the amount of timestamps
                timestamp_end = len(date.timestamp)
               # timestamp_end=
                # without periods last timestamp isnt used
                idx = pd.date_range(
                    timestamp_start, periods=timestamp_end, freq="1H")
                ...
                ax1.xaxis.set_major_locator(
                    dates.MonthLocator(interval=1))    # every Month
                ax1.xaxis.set_major_formatter(
                    dates.DateFormatter('\n%m-%Y'))  # show Month and Year
                # ax1.legend()
                # Place the Legend outside of the graph
                ax1.legend(bbox_to_anchor=(0.7, 1.02, 1, .102),
                ...
                fig1.tight_layout()
                fig1.savefig("Absolute_Irradiance_front_back_" +
            ...
            if SimulationDict["simulationMode"] == 1 or SimulationDict["simulationMode"] == 2:
               ...
               # timestamp_end=
                ...
                ax2.xaxis.set_minor_locator(
                    dates.DayLocator(interval=1))   # every hour
                ax2.xaxis.set_minor_formatter(
                    dates.DateFormatter('%d'))  # hours and minutes
                ax2.xaxis.set_major_locator(
                    dates.MonthLocator(interval=1))    # every day
                ...
                fig2.tight_layout()
                fig2.savefig(
                    "Irradiance_" + datetime.datetime.now().strftime("%Y-%m-%d-%H-%M") + ".png")
                # plt.show()
...
# =============================================================================
        def makePlotBifacialRadiance(resultsPath):
            ...
            if SimulationDict["simulationMode"] == 1 or SimulationDict["simulationMode"] == 2:
                ...
                data = pd.read_csv(resultsPath + "electrical_simulation" +
               ...
               # timestamp_end=
                ...
                ax3.xaxis.set_minor_formatter(
                    dates.DateFormatter('%d'))  # day and hours
                ...
                fig3.tight_layout()
                fig3.savefig("Bifacial_output_Power_" +
                             datetime.datetime.now().strftime("%Y-%m-%d-%H-%M") + ".png")
...
# Entries for default settings
        ...
        def setdefault():
            ...
            Entry_weatherfile.insert(
                0, weatherFile_configfile)  # zu überarbeiten
         ...
         #   Entry_moduley.insert(0, moduley_configfile)
            ...
            # get default value for soiling rate from json file
...
# Entry for delete button
        ...
        Label_Name = ttk.Label(
            namecontrol_frame, text="Insert Simulation Name:", width=20)
        ...
        rad1_weatherfile = Radiobutton(namecontrol_frame, variable=rb_weatherfile,
                                       width=15, text="Local weather File!", value=0, command=lambda: Weatherfile())
        rad2_weatherfile = Radiobutton(namecontrol_frame, variable=rb_weatherfile,  width=20,
                                       text="Download weather File!", value=1, command=lambda: Weatherfile())
        ...
        def InsertReflectivityfile():
            ...
            filename = tk.filedialog.askopenfilename(
                title="Select .csv file", filetypes=(("TMY .csv files", "*.csv"),))
        ...
        Lab_weatherfile = ttk.Label(
            namecontrol_frame, text="Add Path of weatherfile:")
        Lab_weatherfile.grid(row=4, column=0, sticky=W)
        Entry_weatherfile = ttk.Entry(
            namecontrol_frame, background="white", width=25)
        Entry_weatherfile.grid(row=4, column=1)
        Button_weatherfile = ttk.Button(
            namecontrol_frame, text="Insert Weatherfile!", command=InsertWeatherfile)
        Button_weatherfile.grid(row=4, column=2, sticky=W)

        # Changing the reflectivityfile
        Lab_reflectivityfile = ttk.Label(
            namecontrol_frame, text="Add Path of reflectivityfile:")
        Lab_reflectivityfile.grid(row=5, column=0, sticky=W)
        Entry_reflectivityfile = ttk.Entry(
            namecontrol_frame, background="white", width=25)
        Entry_reflectivityfile.grid(row=5, column=1)
        Button_reflectivityfile = ttk.Button(
            namecontrol_frame, text="Insert Reflectivityfile!", command=InsertReflectivityfile)
        ...
        Label_longitude.grid(column=0, row=7, sticky=W)
        Label_latitude.grid(column=0, row=6, sticky=W)
        Entry_longitude = ttk.Entry(
            namecontrol_frame, background="white", width=10)
        Entry_latitude = ttk.Entry(
            namecontrol_frame, background="white", width=10)
        Entry_longitude.grid(column=1, row=7, sticky=W)
        Entry_latitude.grid(column=1, row=6, sticky=W)

        # Setting UTC offset of Longitude and Latitude coordinates
        Calculate_UTC = ttk.Button(
            namecontrol_frame, text="Set UTC offset", command=lambda: Set_UTC_offset())
...
# =============================================================================
     ...
     # Create the different simulationModes 1 to 5
        ...
        r = self.r
        r.set("0")
        rad1_simulationMode = Radiobutton(simulationMode_frame, variable=r, indicatoron=0, width=55,
                                          text='front simulation with Viewfactors, back simulation with Raytracing', value=0, command=lambda: simMode())
        rad2_simulationMode = Radiobutton(simulationMode_frame, variable=r, indicatoron=0, width=55,
                                          text='front and back simulation with Viewfactors', value=1, command=lambda: simMode())
        rad3_simulationMode = Radiobutton(simulationMode_frame, variable=r, indicatoron=0, width=55,
                                          text='front and back simulation with Raytracing', value=2, command=lambda: simMode())  # text='Fixed, Hourly by Timestamps'
        rad4_simulationMode = Radiobutton(simulationMode_frame, variable=r, indicatoron=0, width=55,
                                          text='only front simulation with Viewfactors', value=3, command=lambda: simMode())
        rad5_simulationMode = Radiobutton(simulationMode_frame, variable=r, indicatoron=0, width=55,
                                          text='only back simulation with Raytracing', value=4, command=lambda: simMode())
        ...
        # Inserting Time Data
        Label_startdate = ttk.Label(
            simulationMode_frame, text="Startdate (yy, mm, dd, hh):")
        Label_enddate = ttk.Label(
            simulationMode_frame, text="Enddate (yy, mm, dd, hh):")
        ...
        Entry_year_start = ttk.Entry(
            simulationMode_frame, background="white", width=16)
        Entry_month_start = ttk.Entry(
            simulationMode_frame, background="white", width=4)
        Entry_day_start = ttk.Entry(
            simulationMode_frame, background="white", width=4)
        Entry_hour_start = ttk.Entry(
            simulationMode_frame, background="white", width=4)
        ...
        Entry_year_end = ttk.Entry(
            simulationMode_frame, background="white", width=16)
        Entry_month_end = ttk.Entry(
            simulationMode_frame, background="white", width=4)
        Entry_day_end = ttk.Entry(
            simulationMode_frame, background="white", width=4)
        Entry_hour_end = ttk.Entry(
            simulationMode_frame, background="white", width=4)
        ...
        rad1_rb_SingleAxisTracking = Radiobutton(simulationParameter_frame, variable=rb_SingleAxisTracking,
                                                 width=25, text="Without Single Axis Tracking!", value=0, command=lambda: Singleaxis())
        rad2_rb_SingleAxisTracking = Radiobutton(simulationParameter_frame, variable=rb_SingleAxisTracking,
                                                 width=20, text="With Single Axis Tracking!", value=1, command=lambda: Singleaxis())
        rad1_rb_SingleAxisTracking.grid(column=0, row=1, sticky=W)
        rad2_rb_SingleAxisTracking.grid(
        ...
        rb_Albedo = IntVar()
        rb_Albedo.set("0")
        rad1_Albedo = Radiobutton(simulationParameter_frame, variable=rb_Albedo, width=23,
                                  text="Average measured Albedo!", value=0, command=lambda: Measuredalbedo())
        rad2_Albedo = Radiobutton(simulationParameter_frame, variable=rb_Albedo,  width=23,
                                  text="Hourly measured Albedo!", value=1, command=lambda: Measuredalbedo())
        rad3_Albedo = Radiobutton(simulationParameter_frame, variable=rb_Albedo,  width=20,
                                  text="Hourly spectral Albedo!", value=2, command=lambda: Measuredalbedo())
        rad4_Albedo = Radiobutton(simulationParameter_frame, variable=rb_Albedo,  width=20,
                                  text="Hourly variable Albedo!", value=3, command=lambda: Measuredalbedo())
       ...
       # Defining the electrical Mode with or without Values of rear side
        ...
        rad1_ElectricalMode = Radiobutton(ModuleParameter_frame, variable=rb_ElectricalMode,
                                          width=22, text="OneDiode front and back", value=0, command=lambda: Electricalmode())
        rad2_ElectricalMode = Radiobutton(ModuleParameter_frame, variable=rb_ElectricalMode,
                                          width=21, text="OneDiode with BiFactor", value=1, command=lambda: Electricalmode())
        ...
        rad4_ElectricalMode = Radiobutton(ModuleParameter_frame, variable=rb_ElectricalMode,
                                          width=21, text="TwoDiode with BiFactor", value=3, command=lambda: Electricalmode())
        ...
        rad1_BacktrackingMode = Radiobutton(simulationParameter_frame, variable=rb_Backtracking,
                                            width=19, text="Without Backtracking!", value=0, command=lambda: Backtracking())
        rad2_BacktrackingMode = Radiobutton(simulationParameter_frame, variable=rb_Backtracking,
                                            width=15, text="With Backtracking!", value=1, command=lambda: Backtracking())
...
# =============================================================================
        ...
        # Tilt
        Label_Tilt = ttk.Label(simulationParameter_frame,
                               text="Fixed Tilt of the PV surface:")
        ...
        Entry_Tilt = ttk.Entry(simulationParameter_frame,
                               background="white", width=10)
        Entry_Tilt.grid(column=1, row=2, sticky=W)

        # Limit Angle
        Label_LimitAngle = ttk.Label(
            simulationParameter_frame, text="Limit Angle of Tracking System:")
        Label_LimitAngle.grid(column=0, row=3, sticky=W)
        Label_LimitAnglePar = ttk.Label(
            simulationParameter_frame, text="[Deg°]")
        Label_LimitAnglePar.grid(column=2, row=3, sticky=W)
        Entry_LimitAngle = ttk.Entry(
            simulationParameter_frame, background="white", width=10)
        Entry_LimitAngle.grid(column=1, row=3, sticky=W)

        # hub_height and Clearance Height
        Label_ClearanceHeight = ttk.Label(
            simulationParameter_frame, text="Clearence height:")
        Label_HubHeight = ttk.Label(
            simulationParameter_frame, text="Hub height:")
        Label_ClearanceHeight.grid(column=0, row=5, sticky=W)
        Label_HubHeight.grid(column=0, row=6, sticky=W)
        Label_ClearanceHeightPar = ttk.Label(
            simulationParameter_frame, text="[m]")
        ...
        Entry_ClearanceHeight = ttk.Entry(
            simulationParameter_frame, background="white", width=10)
        Entry_HubHeight = ttk.Entry(
            simulationParameter_frame, background="white", width=10)
        Entry_ClearanceHeight.grid(column=1, row=5, sticky=W)
        Entry_HubHeight.grid(column=1, row=6, sticky=W)

        # Azimuth of the PV surface
        Label_Azimuth = ttk.Label(
            simulationParameter_frame, text="Azimuth of PV surface:")
        Label_Azimuth.grid(column=0, row=7, sticky=W)
        Label_AzimuthPar = ttk.Label(
            simulationParameter_frame, text="[Deg°]" + " (90° = East) (180°= South)")
        Label_AzimuthPar.grid(column=2, row=7, sticky=W)
        Entry_Azimuth = ttk.Entry(
            simulationParameter_frame, background="white", width=10)
        Entry_Azimuth.grid(column=1, row=7, sticky=W)

        # Number of rows and Modules in Rows
        Label_nRows = ttk.Label(
            simulationParameter_frame, text="Number of Rows:")
        Label_nModsx = ttk.Label(
            simulationParameter_frame, text="Number of Modules in Row in X Axis:")
        Label_nModsy = ttk.Label(
            simulationParameter_frame, text="Number of Modules in Row in Y Axis:")
        ...
        Entry_nRows = ttk.Entry(simulationParameter_frame,
                                background="white", width=10)
        Entry_nModsx = ttk.Entry(
            simulationParameter_frame, background="white", width=10)
        Entry_nModsy = ttk.Entry(
            simulationParameter_frame, background="white", width=10)
        ...
        # sensors
        Label_sensors = ttk.Label(
            simulationParameter_frame, text="Number of Sensors:")
        ...
        Entry_sensors = ttk.Entry(
            simulationParameter_frame, background="white", width=10)
        Entry_sensors.grid(column=1, row=11, sticky=W)

        # Lenght of Modules in y and x-axis
        Label_moduley = ttk.Label(
            simulationParameter_frame, text="Length of modules in y-axis:")
        Label_modulex = ttk.Label(
            simulationParameter_frame, text="Length of modules in x-axis:")
        ...
        Entry_moduley = ttk.Entry(
            simulationParameter_frame, background="white", width=10)
        Entry_modulex = ttk.Entry(
            simulationParameter_frame, background="white", width=10)
        ...
        Label_frontReflect = ttk.Label(
            simulationParameter_frame, text="Front surface reflectivity of PV rows:")
        Label_backReflect = ttk.Label(
            simulationParameter_frame, text="Back surface reflectivity of PV rows:")
        Label_frontReflect.grid(column=0, row=14, sticky=W)
        Label_backReflect.grid(column=0, row=15, sticky=W)
        Label_frontReflectPar = ttk.Label(
            simulationParameter_frame, text="[-]")
        ...
        Entry_frontReflect = ttk.Entry(
            simulationParameter_frame, background="white", width=10)
        Entry_backReflect = ttk.Entry(
            simulationParameter_frame, background="white", width=10)
        Entry_frontReflect.grid(column=1, row=14, sticky=W)
        Entry_backReflect.grid(column=1, row=15, sticky=W)

        # gcr
        Label_gcr = ttk.Label(simulationParameter_frame,
                              text="Ground coverage ratio:")
        Label_gcr.grid(column=0, row=16, sticky=W)
        Label_gcrPar = ttk.Label(
            simulationParameter_frame, text="[-] (module area / land use)")
        Label_gcrPar.grid(column=2, row=16, sticky=W)
        Entry_gcr = ttk.Entry(simulationParameter_frame,
                              background="white", width=10)
...
# =============================================================================
        ...
        Entry_bi_factor = ttk.Entry(
            ModuleParameter_frame, background="white", width=8)
        ...
        Entry_Iscf = ttk.Entry(ModuleParameter_frame,
        ...
        Entry_Iscr = ttk.Entry(ModuleParameter_frame,
                               background="white", width=8)
        ...
        Entry_Vocf = ttk.Entry(ModuleParameter_frame,
                               background="white", width=8)
        ...
        Entry_Vocr = ttk.Entry(ModuleParameter_frame,
                               background="white", width=8)
        ...
        Entry_Vmppf = ttk.Entry(ModuleParameter_frame,
                                background="white", width=8)
        ...
        Entry_Vmppr = ttk.Entry(ModuleParameter_frame,
                                background="white", width=8)
        ...
        Entry_Imppf = ttk.Entry(ModuleParameter_frame,
                                background="white", width=8)
        ...
        Entry_Imppr = ttk.Entry(ModuleParameter_frame,
                                background="white", width=8)
        ...
        Entry_Pmpp = ttk.Entry(ModuleParameter_frame,
                               background="white", width=8)
        ...
        Entry_Tamb = ttk.Entry(ModuleParameter_frame,
                               background="white", width=8)
        ...
        Entry_zeta = ttk.Entry(ModuleParameter_frame,
                               background="white", width=8)
        ...
        Entry_Ns = ttk.Entry(ModuleParameter_frame,
                             background="white", width=8)
...
# =============================================================================
       ...
       # simulationMode_configfile=parser.get('default', 'simulationMode')
        ...
        ClearanceHeight_configfile = parser.getfloat(
            'default', 'clearance_height')
...
# =============================================================================
        ...
        def getAlbedoJSONlist():
            ...
            # Laden des Json FIle aus dem Ordner
            with open(rootPath + '\Lib\input_albedo\Albedo.json') as file:
                jsondata_albedo = json.load(file)

            # Ohne können die Module nicht ausgewählt werden
            systemtuple = ('',)
            for key in jsondata_albedo.keys():  # um auf die Modul Keys zurückgreifen zu können
                # build the tuple of strings
                systemtuple = systemtuple + (str(key),)
            Combo_Albedo['values'] = systemtuple[1:]
            # Combobox auf das erste Modul setzen
            Combo_Albedo.current(0)
        ...
        Entry_albedo = ttk.Entry(
            simulationParameter_frame, background="white", width=10)
        Entry_albedo.grid(column=1, row=19, sticky="w")
        entry_albedo_value = tk.StringVar()
        Combo_Albedo = ttk.Combobox(
            simulationParameter_frame, textvariable=entry_albedo_value)
...
# =============================================================================
        ...
        def getModuleJSONlist():
            ...
            # Laden des Json FIle aus dem Ordner
            with open(rootPath + '\Lib\input_module\module.json') as file:
        ...
        Combo_Module = ttk.Combobox(
            ModuleParameter_frame, textvariable=entry_modulename_value)
        ...
        def logo():
            self.logo = Image.open(
                rootPath+'\Lib\logos\logo_BifacialSimu_transparentresized.png')
            logo = self.logo
            # resizing the image
...
# =============================================================================
        ...
        def Break_Simulation():
            ...
            answer = messagebox.askyesno(
                "Stopping Simulation", "Are you sure you want to stop the simulation?", )

            if answer == 1:
                # breaking flag switches to True to trigger the simulation break
                globals.thread_break = True

        Button_startSimulation = ttk.Button(
            simulationFunction_frame, text="Start Simulation!", command=generate_thread)
        Button_startSimulation.grid(column=2, row=1)
        Button_setDefault = ttk.Button(
            simulationFunction_frame, text="set default!", command=setdefault)
        Button_setDefault.grid(column=0, row=1)
        Button_clear = ttk.Button(
            simulationFunction_frame, text="clear!", command=clearall)
        Button_clear.grid(column=1, row=1)
        Button_stopSimulation = ttk.Button(
            simulationFunction_frame, text="Stop Simulation!", command=Break_Simulation)
    ...
    def makePlotBifacialRadiance(resultsPath, Bifacial_gain):
        ...
        if SimulationDict["simulationMode"] == 1 or SimulationDict["simulationMode"] == 2:
            ...
            ax3.set_ylabel('Power Output\n[W/m²]', size=17)
            ax3.set_xlabel("Time", size=17)
            ax3.set_title('Bifacial Output Power\Bifacial Gain: ' +
                          str(Bifacial_gain*100) + " %", size=18)
